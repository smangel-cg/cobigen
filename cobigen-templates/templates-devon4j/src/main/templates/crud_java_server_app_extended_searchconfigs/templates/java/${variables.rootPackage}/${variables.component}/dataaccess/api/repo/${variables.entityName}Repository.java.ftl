    ...
    <#list pojo.fields as field>
      <#compress>
        ...
        <#if !JavaUtil.isCollection(classObject, field.name)>
          ...
          <#elseif JavaUtil.isNumber(classObject, field.name)>
              ${newFieldType} ${field.name} = criteria.${DevonfwUtil.resolveIdGetter(field,false,"")};
              if (${field.name} != null) {
                QueryUtil.get().whereNumber(query, $(alias.${DevonfwUtil.resolveIdGetter(field, true, pojo.package)}), criteria.get${field.name?cap_first}Option());
              }

          <#elseif JavaUtil.isComparable(classObject, field.name)>
              ${newFieldType} ${field.name} = criteria.${DevonfwUtil.resolveIdGetter(field,false,"")};
              if (${field.name} != null) {
                QueryUtil.get().whereComparable(query, $(alias.<#if field.type=='boolean'>is${fieldCapName}()<#else>${DevonfwUtil.resolveIdGetter(field, true, pojo.package)}</#if>), criteria.get${field.name?cap_first}Option());
              }

          <#else>
              ${newFieldType} ${field.name} = criteria.${DevonfwUtil.resolveIdGetter(field,false,"")};
              if (${field.name} != null) {
                <#-- Checking for boolean is no longer needed here, as boolean is comparable -->
                query.where($(alias.${DevonfwUtil.resolveIdGetter(field, true, pojo.package)}).eq(${field.name}));
              }
